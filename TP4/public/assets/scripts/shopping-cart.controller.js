...
(function($, shoppingCartService, utils) {
	...
	itemsCount.done(function(a) {
		var countElement = $(".shopping-cart").find(".count");
		if (a > 0) {
		    countElement.addClass("visible").text(a);
		} else {
    ...
    function _updateTotalAmount() {
        $("#total-amount").html(utils.formatPrice(shoppingCartService.getTotalAmount()));
    }
    ...
    function _renderShoppingCartView(items) {
        ...
        items.forEach(function(item) {
            ...
            function updateQuantity(quantity) {
                ...
                shoppingCartService.updateItemQuantity(product.id, quantity).done(function() {
                    /* _updateCount();
                     _updateTotalAmount();*/
            ...
            rowElement.find(".remove-item-button").click(function() {
                if (confirm("Voulez-vous supprimer le produit du panier?")) {
                    shoppingCartService.removeItem(product.id).done(function() {
                        render();
                    });
                }
            });
    ...
    function _createItemElement(item) {
        return $("<tr>" +
            ...
            "</div>" +
            "</td>" +
            "<td class='price'>" + utils.formatPrice(item.quantity * item.product["price"]) + "</td>" +
            "</tr>");
    }
    ...
    $("#add-to-cart-form").submit(function(event) {
        ...
        shoppingCartService.addItem(productId, +$(this).find("input").val()).always(function(e) {
		if (e.status == 201) {
			var dialog = $("#dialog");
			dialog.fadeIn();
			setTimeout(function() {
